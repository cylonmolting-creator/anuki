...
**Stop Hook Architecture (CRITICAL — 2026-04-18 fix)**:
- Stop hook stdin does **NOT** contain `last_assistant_message` — Claude Code does not send this field
- The last message must be read from `transcript_path` JSONL file using `get_last_message()` from `scripts/hook-helper.sh`
- Hook output must use `"decision": "approve"` or `"block"` — `"allow"` is **INVALID**
- All stop hooks use `scripts/hook-helper.sh` (emit_block, emit_approve, get_last_message)
- `check_hook_active` guard is mandatory — prevents infinite loops
- macOS does not have `tac` — use `tail -r` instead
- The generator (build-rules.js) has been updated to use this helper automatically

**How Stop hooks work:**
After every agent response, a shell script reads the last assistant message from the session transcript JSONL and scans it for `stop_patterns`. If patterns are found and the rule's enforcement context is not met (e.g., no evidence for claim rules, or pattern match alone for behavioral rules), the response is **BLOCKED**. The agent must rewrite without the forbidden patterns. This is mechanical — the agent cannot bypass it.
...
- **006**: No sloppy work — think before you write (severity: high) — `soul-safety-inject`
- **007**: Push requires runtime verification (severity: critical) — `soul-safety-inject`
