...
function parseRules() {
  ...
  for (const f of files) {
    ...
    rules.push({
      ...
      // Pre-push hook fields
      push_check: meta.push_check || null,
      push_start_cmd: meta.push_start_cmd || null,
      push_health_url: meta.push_health_url || null,
      push_timeout_sec: meta.push_timeout_sec || null,
      push_fail_message: meta.push_fail_message || null,
      body,
    });
  }
  return rules;
...
function generateHookEntries(rules) {
  ...
  // Pre-push hooks (git-level enforcement)
  const prePushRules = rules.filter(r => r.enforcement.includes('pre-push-hook') && r.push_check);

  return { preToolUse, needsReadTracker, stopHooks, prePushRules };
}

/**
 * Build .git/hooks/pre-push script from rules with pre-push-hook enforcement.
 * Idempotent — writes between SSOT markers, preserves any manual blocks outside.
 * Runtime-verified check sequence: start server -> health curl -> tests -> cleanup.
 */
function generatePrePushHookScript(prePushRules) {
  if (!prePushRules || prePushRules.length === 0) return null;

  const ruleIds = prePushRules.map(r => r.id).join(', ');
  const blocks = prePushRules.map(rule => {
    const check = rule.push_check || 'runtime-verified';
    const startCmd = rule.push_start_cmd || 'node src/index.js';
    const healthUrl = rule.push_health_url || 'http://localhost:3000/api/health';
    const timeout = rule.push_timeout_sec || 10;
    const failMsg = (rule.push_fail_message || `Rule ${rule.id} blocked push: runtime verification failed`).replace(/"/g, '\\"');

    if (check === 'runtime-verified' || check === 'health-only') {
      const runTests = check === 'runtime-verified';
      return `
# --- Rule ${rule.id}: ${rule.title.replace(/[\r\n"]/g, '')} ---
echo "[pre-push] Rule ${rule.id}: starting runtime verification..."
LOG=$(mktemp); PIDF=$(mktemp)
${startCmd} > "$LOG" 2>&1 &
echo $! > "$PIDF"
sleep ${timeout}
HTTP=$(curl -s -o /tmp/.prepush-health-${rule.id}.json -w "%{http_code}" "${healthUrl}" || echo 000)
if [ "$HTTP" != "200" ] || ! grep -q '"status":"ok"' /tmp/.prepush-health-${rule.id}.json 2>/dev/null; then
  echo ""
  echo "PRE-PUSH BLOCKED (Rule ${rule.id}): ${failMsg}"
  echo "Health endpoint: ${healthUrl} -> HTTP $HTTP"
  echo "--- last 30 log lines ---"
  tail -30 "$LOG"
  kill "$(cat $PIDF)" 2>/dev/null; rm -f "$LOG" "$PIDF" /tmp/.prepush-health-${rule.id}.json
  exit 1
fi
${runTests ? `if grep -q '"test"[[:space:]]*:' package.json 2>/dev/null; then
  if ! (npm test --silent 2>&1 | tail -30); then
    echo ""
    echo "PRE-PUSH BLOCKED (Rule ${rule.id}): tests failed"
    kill "$(cat $PIDF)" 2>/dev/null; rm -f "$LOG" "$PIDF" /tmp/.prepush-health-${rule.id}.json
    exit 1
  fi
fi` : ''}
kill "$(cat $PIDF)" 2>/dev/null; rm -f "$LOG" "$PIDF" /tmp/.prepush-health-${rule.id}.json
echo "[pre-push] Rule ${rule.id}: PASS"
`;
    } else if (check === 'tests-only') {
      return `
# --- Rule ${rule.id}: tests-only ---
if ! npm test --silent; then
  echo "PRE-PUSH BLOCKED (Rule ${rule.id}): ${failMsg}"
  exit 1
fi
`;
    } else if (check.startsWith('custom:')) {
      const customCmd = check.substring('custom:'.length).replace(/"/g, '\\"');
      return `
# --- Rule ${rule.id}: custom ---
if ! ${customCmd}; then
  echo "PRE-PUSH BLOCKED (Rule ${rule.id}): ${failMsg}"
...
`;
  ...
  return `#!/bin/bash
# SSOT-PREPUSH-START (auto-generated by build-rules.js - rules: ${ruleIds})
# DO NOT EDIT between markers - modify rules/*.md and re-run build-rules.js
set -e

[ -n "$SKIP_SSOT_PREPUSH" ] && { echo "[pre-push] SSOT checks SKIPPED via SKIP_SSOT_PREPUSH env"; exit 0; }

ROOT="$(git rev-parse --show-toplevel)"
cd "$ROOT"
${blocks}
echo "[pre-push] All SSOT runtime checks passed"
exit 0
# SSOT-PREPUSH-END
`;
}
...
/**
 * Write pre-push hook if needed. Returns { changed: bool }.
 */
function updatePrePushHook(prePushRules) {
  const hooksDir = path.join(ROOT, '.git', 'hooks');
  const hookPath = path.join(hooksDir, 'pre-push');

  if (!fs.existsSync(path.join(ROOT, '.git'))) {
    log('  [skip] .git/ not found - no pre-push hook generated');
    return { changed: false };
  }

  const script = generatePrePushHookScript(prePushRules);
  if (!script) {
    // No pre-push rules -> remove SSOT block from hook if it exists
    if (fs.existsSync(hookPath)) {
      const existing = fs.readFileSync(hookPath, 'utf8');
      if (existing.includes('SSOT-PREPUSH-START')) {
        const cleaned = existing.replace(/# SSOT-PREPUSH-START[\s\S]*?# SSOT-PREPUSH-END\n?/g, '').trim();
        if (DRY_RUN) {
          log('  [would-write] .git/hooks/pre-push (SSOT block removed)');
        } else if (cleaned.trim() === '#!/bin/bash' || cleaned.trim() === '') {
          fs.unlinkSync(hookPath);
          log('  [delete] .git/hooks/pre-push (no rules, hook removed)');
        } else {
          fs.writeFileSync(hookPath, cleaned + '\n', { mode: 0o755 });
          log('  [write] .git/hooks/pre-push (SSOT block removed, manual content kept)');
        }
        return { changed: true };
      }
    }
  ...
  let finalContent;
  if (existing.includes('SSOT-PREPUSH-START')) {
    // Replace SSOT block, preserve manual content outside
    finalContent = existing.replace(/# SSOT-PREPUSH-START[\s\S]*?# SSOT-PREPUSH-END\n?/g, script.replace(/^#!\/bin\/bash\n/, '') + '\n');
    if (!finalContent.startsWith('#!/bin/bash')) finalContent = '#!/bin/bash\n' + finalContent;
  } else {
    finalContent = script;
  }

  if (existing === finalContent) return { changed: false };

  if (DRY_RUN) {
    log(`  [would-write] .git/hooks/pre-push (${prePushRules.length} rule(s))`);
  } else {
    fs.writeFileSync(hookPath, finalContent, { mode: 0o755 });
    log(`  [write] .git/hooks/pre-push (${prePushRules.length} rule(s), ${finalContent.length} bytes)`);
  }
  return { changed: true };
}
...
function main() {
  ...
  // Pre-push hook (git-level enforcement)
  log('\n=== Pre-push hook (.git/hooks/pre-push) ===');
  const r4 = updatePrePushHook(hookData.prePushRules);
  if (r4.changed) totalChanged++;
  ...
  log(`Pre-push rules: ${hookData.prePushRules ? hookData.prePushRules.length : 0}`);
