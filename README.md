...
`http://localhost:3000` — bound to localhost by default. Do **not** expose Anuki directly to the public internet. If you need remote access, put it behind an authenticated reverse proxy (Tailscale, Cloudflare Tunnel with auth, VPN, etc.) and set `API_AUTH_TOKEN` in `.env` so the `/api/*` endpoints require a bearer token.
